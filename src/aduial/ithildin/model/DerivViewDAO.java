...
public class DerivViewDAO {
  ...
  public static ObservableList<DerivView> getEntryDerivViewList(int id) {
    ...
    String selectStmt = "SELECT * FROM derivview dv "
            + "WHERE dv.entryidfrom = " + id + " "
            + "AND dv.refgloss is not null;";
    try {
      ...
      System.out.println("Error fetching the Deriv view type linked to Entry / Lexicon "
              + "with id = " + id + ": " + e);
    }

...
public class LexiconDAO {
  ...
  public static Lexicon fetchLexicon(Integer id) throws
    ...
    try {
      ...
      System.out.println("While fetching the lexicon with id = " + id.toString()
              + " an error occurred: " + e);
      throw e;
    }
  }
  ...
  public static String getSpeech(int id) {
    String selectStmt = "SELECT g.* FROM GRAMMAR g "
            + "JOIN LINKED_GRAMMAR lg ON g.ID = lg.GRAMMAR_ID "
            + "JOIN TYPE t ON lg.GRAMMARTYPE_ID = t.ID "
            + "WHERE lg.ENTRY_ID = " + id + " "
            + "AND t.TXT = 'speechform';";
    try {
      ResultSet rs = DBUtil.dbExecuteQuery(selectStmt);
      String speech = "";
      if (rs.next()) {
      ...
      System.out.println("Error fetching the Speech grammar type linked to Entry / Lexicon "
              + "with id = " + id + ": " + e);
      return "-";
    }
  }
  ...
  public static ObservableList<EntryNoteView> getEntryNoteList(int id) {
    ...
    String selectStmt = "SELECT * FROM entrynoteview env "
            + "WHERE env.ENTRY_ID = " + id + ";";
    try {
      ResultSet rs = DBUtil.dbExecuteQuery(selectStmt);
      while (rs.next()) {
      ...
      System.out.println("Error fetching the Entry Note (doc) type linked to Entry / Lexicon "
              + "with id = " + id + ": " + e);
    }

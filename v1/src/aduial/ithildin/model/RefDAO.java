...
public class RefDAO{
    ...
    public static ObservableList<Ref> getRefList(int entryId) {
        ...
        String selectStmt = "SELECT * FROM REF " + "WHERE ENTRY_ID = " + entryId + ";";
        Ref ref = null;
        try {
            ...
            System.out.println(
                    "Error fetching the Refs linked to Entry / Lexicon " + "with id = " + entryId + ": " + e);
        }
        return refList;
    }
    ...
    public static ObservableList<String> getRefGlossList(int entryId) {
        ...
        String selectStmt = "SELECT '\"' || g.txt || '\" ✧ ' || Group_Concat(r.source, '; ') refgloss " + "FROM ref r " + "JOIN gloss g ON g.id = r.gloss_id " + "WHERE r.entry_id = " + entryId + " " + "AND NOT EXISTS (select * FROM linked l WHERE l.ref_id = r.id) " + "GROUP BY LOWER(g.txt);";
        String refGloss;
        try {
            ResultSet rs = DBUtil.dbExecuteQuery(selectStmt);
            while (rs.next()) {
            ...
            System.out.println(
                    "Error fetching the Ref Glosses for Entry / Lexicon " + "with id = " + entryId + ": " + e);
        }
        return refGlossList;
    }
